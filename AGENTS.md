...
A `Backup`/`Restore`/`BackupGroups`/`RestoreGroups` error does not abort the surrounding command: `runBackupWithService`/`runRestoreWithService` log it via `service.logger.Log("ERROR", ...)` and let the rest of the pipeline run to completion, then return that error at the end (unless a later step also fails, in which case the later error takes precedence as the return value — the backend error's `ERROR` line has already been written either way). Concretely: a `runRestore` backend failure still stops/transfers/restarts containers using whatever ended up in the staging directory, and a `runBackup` backend failure (already the last step) just means the failure is now recorded as an `ERROR` log line instead of only surfacing as Cobra's raw stderr dump. Either way the process still exits non-zero — this is "log and continue," not "swallow," matching `StartStoppedContainers`' per-container log-and-continue pattern in `internal/shared/container_lifecycle.go`. This means, for restore specifically, that a failed `Backend.Restore()` no longer prevents containers from being stopped/restarted — a deliberate choice to prioritize container uptime over aborting on a storage-layer failure the operator can address separately; if backend data was genuinely incomplete, the transfer step still runs against whatever is in the staging directory, so check the `ERROR` log line before trusting the result of a restore that reported this kind of failure.

Both `resolveBackend` functions (`cmd/backup/backup.go`, `cmd/restore/restore.go`) are declared as a package-level `var` holding a func value, not a plain `func`, purely as a test seam — it lets `orchestration_test.go` swap in a fake `backend.Backend` (via a `withResolveBackend` test helper mirroring `withBackupDirs`/`withRestoreDirs`) to exercise the log-and-continue path without going through `Factory.GetBackend`. This has no runtime effect; production code still calls it exactly as before.
...
| `azure` | `azure:<container>:/<path>` | `AZURE_ACCOUNT_KEY` | `AZURE_ACCOUNT_NAME`, `AZURE_ENDPOINT_SUFFIX` (optional) |
| `gcs` | `gs:<bucket>:/<path>` | — | `GOOGLE_PROJECT_ID`, `GOOGLE_APPLICATION_CREDENTIALS`, `STORAGE_EMULATOR_HOST`/`GOOGLE_ACCESS_TOKEN` (optional, paired — see below) |
...
- **Two `azure`/`gcs` env vars were missing, causing the CI integration tests to fail against real endpoints instead of Azurite/fake-gcs-server.** Confirmed by reading restic 0.19.1's actual `internal/backend/azure`/`internal/backend/gs` source (not assumed): (1) `azure.Storage` now also sends `AZURE_ENDPOINT_SUFFIX` (from a new `EndpointSuffix`/`endpoint_suffix` field, restic's equivalent of kopia's `--storage-domain`) when set — without it, restic defaults to real Azure's `core.windows.net`, and since `*.blob.core.windows.net` is a live wildcard DNS across Azure's real frontend fleet, a CI runner with real internet access doesn't fail closed (DNS/connection error) but actually reaches real Azure and gets a live, real 403 auth rejection for Azurite's well-known dev account/key — a confusing failure that looks like a credentials bug but is actually a missing endpoint override. (2) `gcs.Storage` now also sends a dummy `GOOGLE_ACCESS_TOKEN` alongside `STORAGE_EMULATOR_HOST` when `EmulatorHost` is set: restic's own `getStorageClient()` unconditionally calls `google.DefaultTokenSource()` — and fails with "could not find default credentials" — unless `GOOGLE_ACCESS_TOKEN` is already set; only past that does it reach `cloud.google.com/go/storage`'s `NewClient`, which is the thing that actually honors `STORAGE_EMULATOR_HOST` (restic's own `gs.go` never references that variable). Both fixture configs (`test/config.restic-azure.json`, `test/config.restic-gcs.json` implicitly via code) and both storage-type unit tests were updated to match. Azure's fix is only a partial win, though — see the next bullet.
- **`azure`'s Azurite incompatibility is real and upstream, matching kopia's own documented limitation.** Even with `AZURE_ENDPOINT_SUFFIX` correctly pointed at Azurite, restic's azure backend (like kopia's) always builds a virtual-hosted-style **HTTPS** URL (`https://<account>.blob.<suffix>/<container>`) with no override for scheme or path-style addressing, while Azurite serves plain-HTTP path-style URLs — the connection reaches Azurite (DNS resolves `*.localhost` to loopback) but then fails the TLS handshake with Go's standard `server gave HTTP response to HTTPS client` error. `storage/azure/integration_azure_test.go`'s `TestIntegration_Azure` checks for that exact, stable error string and skips with an explanation instead of failing — mirroring kopia's own `isKnownAzuriteAddressingIncompatibility` pattern in `internal/backend/kopia/storage/azure/integration_azure_test.go` — rather than either failing permanently or skipping unconditionally (which would hide a real regression).
- **`sftp`'s committed private key (`test/restic_sftp_key/id_ed25519`) needs 0600 permissions ssh will actually accept, but git only tracks the executable bit, not the rest of the mode** — a fresh checkout (e.g. in CI) restores it as 0644, and `ssh` refuses to use a private key that's group/world-readable ("Permissions 0644 ... are too open"). `TestIntegration_SFTP` no longer points `KeyfilePath` at the checked-out file directly; it copies the key's bytes into a fresh `t.TempDir()` file written with `0o600`, the same pattern already used for the live-fetched `known_hosts` file.
...
- **`TestIntegration_Azure` appeared to hang rather than skip, because restic retries every backend error for up to 15 minutes before giving up, and the skip-detection string it was waiting for was also wrong.** Confirmed by reproducing directly against a plain HTTP listener standing in for Azurite, using the real `restic` 0.19.1 binary: (1) restic wraps every backend in a retry loop with a hardcoded `retry.New(be, 15*time.Minute, ...)` (`internal/global/global.go` in restic's own source) — there is no CLI flag or env var to shorten this; `--stuck-request-timeout` (default 5m) only covers requests that never get a response at all, not ones that fail fast and get retried, which is what happens here. `internal/backend/restic/integration_helpers.go`'s `capturingCommandRunner` now runs every restic invocation under `exec.CommandContext` with a `commandTimeout` (60s) well under that 15-minute ceiling, so a persistent-looking error like Azurite's TLS mismatch returns in well under a minute instead of blocking for up to 15. (2) Separately, `isKnownAzuriteAddressingIncompatibility`'s string match (`"server gave HTTP response to HTTPS client"`) was never actually what restic returns: that friendlier string is `net/http.ErrSchemeMismatch`, only substituted for the raw `tls.RecordHeaderError` inside `net/http.Client.Do` (`src/net/http/client.go`) — the Azure SDK's own pipeline (what restic's azure backend actually uses) doesn't go through `Client.Do`, so the real, observed error is always the raw, un-substituted `crypto/tls` message, `"tls: first record does not look like a TLS handshake"` (`src/crypto/tls/conn.go`). The matcher now checks for both, so the test still recognizes a genuine scheme-mismatch response if some future restic version's HTTP path ever does produce the friendlier string. Kopia's own `internal/backend/kopia/storage/azure/integration_azure_test.go` has the identical `isKnownAzuriteAddressingIncompatibility` checking only the friendlier string — worth rechecking against a real `kopia` binary, since it may have the same latent bug, but that wasn't verified as part of this fix.
