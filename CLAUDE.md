...
- **Backup vs restore direction**: backup defaults to `backup_src_dir` → `backup_dst_dir`; restore defaults to `backup_dst_dir` → `backup_src_dir` (the config's src/dst fields are named from backup's perspective; restore just reverses them unless overridden by flags).
